...
object Day15 extends AocDay[Array[Array[Int]], Int]("data/day15"):
  ...
  def solve(grid: Array[Array[Int]]): Int =
    ...
    product(range(0, R - 1), range(0, C - 1))
      .foreach((r, c) => distance((r, c)) = Integer.MAX_VALUE)
    val s = (0, 0)
    pq.addOne(s, 0)
    distance(s) = 0
  ...
  def solve2(input: Array[Array[Int]]): Int =
    ...
    product(range(0, 4), range(0, 4)).map((kr, kc) =>
      product(range(0, R - 1), range(0, C - 1)).foreach((r, c) =>
        grid(kr * R + r)(kc * C + c) = (input(r)(c) - 1 + kr + kc) % 9 + 1
      )
    )
    solve(grid)
