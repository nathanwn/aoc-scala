...
object Day19 extends AocDay[Array[List[Vec]], Int]("data/day19"):
  ...
  class Vec(val n: Int, val keys: Array[Int]) extends Ordered[Vec]:
    ...
    def compare(that: Vec): Int = {
      for (i <- 0 until this.n)
        if this(i) < that(i) then return -1
        else if this(i) > that(i) then return 1
      0
    }
    ...
    override def equals(that: Any): Boolean =
      that match
        case that: Vec =>
          for (i <- 0 until this.n)
            if this(i) != that(i) then return false
          true
        case _ => false
  ...
  def connect(us: List[Vec], vs0: List[Vec], B: SqMat): Option[Vec] =
    ...
    product(us, vs).find((u0: Vec, v0: Vec) =>
  ...
  def solve2(scanners: Array[List[Vec]]): Int =
    ...
    product(coors, coors).map((u: Vec, v: Vec) => u.distManhattan(v)).max