...
object Day14
    extends AocDay[(Array[Char], Map[String, Char]), Long]("data/day14"):
  ...
  def solveSlow(steps: Int)(input: (Array[Char], Map[String, Char])): Int =
    ...
    range(0, steps - 1).foreach(_ =>
      val insertions = range(0, s.length - 1)
        .map(i =>
  ...
  def solveFast(steps: Int)(input: (Array[Char], Map[String, Char])): Long =
    ...
    val pairs: List[String] =
      range(0, s.length - 2).map(i => s"${s(i)}${s(i + 1)}")
    ...
    val countMap = pairs
      ...
      .foldLeft(mutable.Map[Char, Long]())((lhs, rhs) =>
        lhs ++ rhs.map { case (k, v) => k -> (v + lhs.getOrElse(k, 0L)) }
      )
