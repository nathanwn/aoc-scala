...
object Day18 extends AocDay[List[Node], Int]("data/day18"):
  ...
  def parse(input: String): List[Node] =
    def parseNode(parent: Option[InnerNode])(input: String): Option[Node] =
      def findDelim(): Int =
        var counter = 0
        var res = -1
        range(1, input.length - 2).foreach(i =>
          input(i) match
            case '[' => counter += 1
            case ']' => counter -= 1
      ...
      if delimPos != -1 then
        ...
        node.right =
          parseNode(Some(node))(input.substring(delimPos + 1, input.length - 1))
        Some(node)
            ...
            if level >= 4 then
              ...
              innerNode.parent match
                case Some(parent) =>
                  ...
                  else parent.right = Some(LeafNode(Some(parent), 0))
                case None =>
              true
            else false
  ...
  def solve2(trees: List[Node]): Int =
    val a = trees.toArray
    product(range(0, trees.length - 1), range(0, trees.length - 1))
      .map((i, j) =>
        if i == j then 0
        else
