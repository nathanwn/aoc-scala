...
object Day13 extends AocDay[(List[(Int, Int)], List[(Int, Int)]), Int]("data/day13"):
  val VER = 0
  val HOR = 1

  def parse(input: String): (List[(Int, Int)], List[(Int, Int)]) =
    val parts = input.split("\n\n")
    val dots = parts(0).split("\n").toList.map(dot =>
      val coors = dot.split(",")
      (coors(0).toInt, coors(1).toInt)
    )
    val cmds = parts(1).split("\n").toList.map(line =>
  ...
  def fold(cmd: (Int, Int))(grid: Array[Array[Boolean]]): Array[Array[Boolean]] =
    if cmd(0) == HOR
  ...
  def foldVer(X: Int)(grid: Array[Array[Boolean]]): Array[Array[Boolean]] =
    ...
    product(range(0, X - 1), range(0, Y - 1)).map((x, y) =>
      newGrid(x)(y) = if 2*X - x >= grid.length
        then grid(x)(y)
  ...
  def foldHor(Y: Int)(grid: Array[Array[Boolean]]): Array[Array[Boolean]] =
    ...
    val newGrid = Array.ofDim[Boolean](X, Y)
    product(range(0, X - 1), range(0, Y - 1)).map((x, y) =>
      newGrid(x)(y) = if 2*Y - y >= grid(0).length
        then grid(x)(y)
