...
object Day23 extends AocDay[Array[Array[Char]], Long]("data/y2021/Day23"):
  ...
  class Node(val grid: Array[Array[Char]]):
    ...
    def adjacencyList: List[Edge] =
      ...
      def roomRow(cTo: Int): Int =
        cRoom.find(_._2 == cTo).map(_._1) match
          case None => -1
          case Some(agent) =>
            (2 to grid.length - 2).findLast(this(_, cTo) == '.') match
              case None => -1
              case Some(rTo) =>
      ...
      def fromRoomToHallway(fromCell: Cell): List[Edge] =
        ...
        if rFrom > 2 && (2 to rFrom - 1).exists(this(_, cFrom) != '.') then
          return List()
      ...
      val roomCells: Seq[(Int, Int)] =
        ...
        yield (r, c))
          .filter((r, c) =>
            c != cRoom(this((r, c))) ||
              (r < grid.length - 2 && (r + 1 to grid.length - 2).exists(rr =>
                !cRoom.contains(this(rr, c)) || cRoom(this(rr, c)) != c
              ))
          )
      val hallwayToRoomEdges: Seq[Edge] =
        hallwayCells.flatMap(fromHallwayToRoom)
      val roomToHallwayEdges: Seq[Edge] = roomCells.flatMap(fromRoomToHallway)
    ...
    def isFinal: Boolean =
      agents.forall(agent =>
        (2 to grid.length - 2).forall(r => this(r, cRoom(agent)) == agent)
      )
