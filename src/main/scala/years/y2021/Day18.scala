    ...
    def parse(input: String): List[Node] =
        def parseNode(parent: Option[InnerNode])(input: String): Option[Node] =
            ...
            if delimPos != -1 then
                ...
                node.right = parseNode(Some(node))(
                  input.substring(delimPos + 1, input.length - 1)
                )
                Some(node)
            else Some(LeafNode(parent, Integer.parseInt(input)))
        ...
        node match
            ...
            case leafNode: LeafNode =>
                if leafNode.value < 10 then false
                    ...
                    splitNode.left = Some(
                      LeafNode(Some(splitNode), leafNode.value / 2)
                    )
                    splitNode.right = Some(
                      LeafNode(Some(splitNode), (leafNode.value + 1) / 2)
                    )
                    leafNode.parent match
                        case Some(parent) =>
                            if parent.left.contains(leafNode) then
                                parent.left = Some(splitNode)
                            else parent.right = Some(splitNode)
                        case None =>
                    true
            case _ => throw new java.lang.RuntimeException()
